...
function(input, output, session) {
  # Reactive expression to choose the table based on input selection
  ...
  # Render the selected table using DT
  output$cohortTable <- renderDT({
    table_data <- selected_table()
    
    # Replace all NAs with blank for the entire data frame
  ...
  # Dynamically render description text based on table choice
  output$descriptionText <- renderText({
    switch(input$tableChoice,
           ...
           "cohort_revenue" = "This table displays the revenue for each cohort, showing the total revenue generated by customers in each cohort."
    )
  })
}
