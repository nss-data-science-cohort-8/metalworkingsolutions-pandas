...
p <- ggplot(df_long, aes(x = year_month, y = count, fill = company_type)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(title = "Number of New vs Existing Companies Per Month",
       x = "Month",
...
# Count unique first-time customers per order date
customers_per_date <- first_time_customers %>%
  group_by(first_purchase_date) %>%
  summarise(num_customers = n()) %>%
  arrange(first_purchase_date)

# Create a categorical variable for legend
customers_per_date$Legend <- "First-Time Customers"

# Create a bar chart with legend
p <- ggplot(customers_per_date, aes(x = first_purchase_date, y = num_customers, fill = Legend)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("First-Time Customers" = "steelblue")) +  # Legend Color
  labs(title = "Number of First-Time Customers Per Order Date",
       x = "Order Date",
